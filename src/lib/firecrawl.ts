...
export class FirecrawlClient {
  ...
  async scrapeUrl(url: string, timeoutMs: number = 15000) {
    try {
      // Handle PDF URLs gracefully
      if (url.toLowerCase().endsWith('.pdf')) {
        return {
          markdown: `This tool cannot directly analyze PDF files. Please provide a link to a web page (e.g., the paper's abstract page on arXiv, a conference site, or a university page).`,
          html: '',
          metadata: {
            error: 'PDF URL detected',
    ...
    try {
      ...
      const enrichedData = data.map((item: any) => {
        ...
        if (!favicon && item.metadata?.ogImage) {
          ...
          favicon = `https://${domain}/brain.png`;
        }
